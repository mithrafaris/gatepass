...
function Dashboard() {
  ...
  const validationSchema = Yup.object({
    PassNumber: Yup.number()
      .required("Pass Number is required")
      .positive("Must be a positive number")
      .integer("Must be an integer"),
    customerName: Yup.string().required("Customer Name is required"),
    customerAddress: Yup.string().required("customerAddress is required"),
    material: Yup.string().required("Material is required"),
    OutDate: Yup.date()
      .required("Out Date is required")
      .typeError("Invalid date format"),
    inDate: Yup.date()
      .required("In Date is required")
      .typeError("Invalid date format")
      .min(Yup.ref("OutDate"), "In Date cannot be earlier than Out Date"),
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to create pass: ${errorData.message}`);
      }
    } catch (error) {
                  ...
                  <Form>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {[
                        ...
                        { name: "customerAddress", label: "customerAddress", type: "text" },
